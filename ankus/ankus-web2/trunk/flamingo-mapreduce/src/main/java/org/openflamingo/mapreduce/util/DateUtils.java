...
public class DateUtils {
    ...
    /**
     * 두 날짜간의 시간차를 반환한다.  이 메소드는 윤년이 적용하여 계산한다.
     *
     * @param from 시작 날짜
     * @param to   끝 날짜
     * @return 시간차
     ...
     * @return 날짜
     * @throws java.io.IOException 날짜 패턴을 파싱할 수 없는 경우
     */
    ...
    /**
     * 기준 날짜의 몇일전 기준일을 구성한다.
     *
     * @param base 기준일
     * @param diff 날짜차
     * @return 날짜차가 적용된 문자열 날짜
     * @throws java.io.IOException 파싱할 수 없는 경우
     */
    public static String formatDate(String base, int diff) throws IOException {
        Date baseDate = DateUtils.formatDate("yyyyMMdd", base);
        Date diffDate = DateUtils.addDays(baseDate, diff);
        return DateUtils.parseDate(diffDate, "yyyyMMdd");
    }
