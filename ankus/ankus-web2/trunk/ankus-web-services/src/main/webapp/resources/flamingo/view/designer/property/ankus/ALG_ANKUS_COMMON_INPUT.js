...
Ext.define('Flamingo.view.designer.property.ankus.ALG_ANKUS_COMMON_INPUT', {
    extend: 'Flamingo.view.designer.property._NODE_ALG',
    alias: 'widget.ALG_ANKUS_COMMON_INPUT',
    requires: [
...
//                    iconCls: 'common-confirm',
                    handler: function (grid, rowIndex, colIndex) {
                        ...
                        var store = Ext.create('Ext.data.Store', {
                            ...
                            proxy: {
                                type: 'ajax',
                                url: CONSTANTS.CONTEXT_PATH + CONSTANTS.DESIGNER.LOAD_HDFS_FILE_TAB,
                                headers: {
                        ...
                        var columnIndexList;
                        var rowDataList;
                        var gridtest = Ext.getCmp('jsonForm');
                        ...
                        store.on('load', function (store, records) {
                            ...
                            for (var i = 0; i < records.length; i++) {
                                ...
                                for (var k = 0; k < rowDataList.length; k++) {
                                	var tempRow = {};
                                	var str_rowData = rowDataList[k].split(',');                                	
                                	
                                	for(var j = 0; j < str_rowData.length ; j++){  
                                		var key = 'column'+j; 
                                		//var key = j; 
                                		if(k==0){
                                		columns.push({ text : 'column'+j, dataIndex : key, flex:1});
                    					fields.push(key);                                		
                                		}
                                		tempRow[key]=str_rowData[j]; 
                                	}  
                                	valData.push(tempRow);
                                }           
                                //store.insert(0, rec);                                 
                            ...
                            restore.loadData(valData);                            
                           // Ext.suspendLayouts();
                        	previewGrid.reconfigure(restore,columns);
                        });                        
                        ...
                        var gridtest = Ext.getCmp('jsonForm');    
                        /*
                        for(i=0 ; i < gridtest.items.keys.length; i++){
                        	
                        	if(gridtest.items.keys[i] == 'indexList'){                        		
                        		reconfigure_model.push(
                    				{
		                                xtype: 'checkcolumn',
		                                width: 55,
		                                header: 'Target',
                        	...
                        	if(gridtest.items.keys[i] == 'nominalIndexList'){                        		
                        		reconfigure_model.push(
                    				{
		                                xtype: 'checkcolumn',
		                                width: 65,
                        	...
                        	if(gridtest.items.keys[i] == 'exceptionIndexList'){                        		
                        		reconfigure_model.push(
                    				{
		                                xtype: 'checkcolumn',
		                                width: 65,
                        	...
                        	if(gridtest.items.keys[i] == 'classIndex'){                        		
                        		reconfigure_model.push(
                    				{
		                                xtype: 'checkcolumn',
		                                width: 65,
                    	...
                    	Ext.suspendLayouts();
                    	previewGrid.reconfigure(store,reconfigure_model);
                        Ext.resumeLayouts(true);
                        
                        for(i=0 ; i < gridtest.items.keys.length; i++){                        	
                        	if(gridtest.items.keys[i] == 'indexList'){   
                        		Ext.ComponentQuery.query('#TargetAll')[0].enable();                        		
                        	}else if(gridtest.items.keys[i] == 'nominalIndexList'){
                        		Ext.ComponentQuery.query('#NominalAll')[0].enable();
                        	}else if(gridtest.items.keys[i] == 'exceptionIndexList'){
                        		Ext.ComponentQuery.query('#ExceptionAll')[0].enable();
                        	}else if(gridtest.items.keys[i] == 'classIndex'){
                        		Ext.ComponentQuery.query('#ResetAll')[0].enable();
                        	}
                        }
                        */
                    }
    ...
    afterPropertySet: function (props) {
        ...
        var jsonParam = Ext.getCmp('jsonForm').items;              
        
        for(i=0; i< jsonParam.length; i++) { 
        	        	
        	 var param = jsonParam.items[i];
        	
        	 if(param.xtype == 'radiogroup'){        		
	    		 var radionm = param.items.get(0).name;
	    		 var radioval = param.items.get(0).getGroupValue();
	    		 
	    		 props.mapreduce.params.push("-" + radionm, radioval);
	    	 }else if(param.xtype == 'combo'){			    		 
	    		 props.mapreduce.params.push("-" + param.name, param.value);
	    	 }        	 
	    	 
	    	 if(param.xtype == 'fieldcontainer' && param.items.items[0].value != ''){	    		 
	    		 props.mapreduce.params.push('-' + param.items.items[0].id, param.items.items[0].value);	 
	    	 }
		    		 
	    	 if(param.xtype == 'textfield' && param.value != ''){
	    	 ...
	    	 /*
	    	 if (props.param) {
	             props.mapreduce.params.push("-"+param , props.param);
	         }
	    	 */
	    } 
