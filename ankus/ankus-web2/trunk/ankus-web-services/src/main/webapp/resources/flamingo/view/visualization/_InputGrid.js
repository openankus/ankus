 ...
 */
Ext.define('Flamingo.view.visualization._InputGrid', {
    extend: 'Flamingo.view.designer.property._Grid',
    alias: 'widget._vinputGrid',
        ...
        this.tools = [
            {
                ...
                handler: function (event, toolEl, panel) {
                    var grid = panel.up('_grid'),  store = grid.getStore();
                    var engine = Ext.getCmp(me.engine);
                    var engineId;
                    
                    if (engine) {
                    	engineId = Ext.getCmp(me.engine).value;
                    } else {
                    	var canvas = Ext.ComponentQuery.query('canvas')[0];
                    	var form = canvas.getForm();
                    	engineId = form.getValues()['engine_id'];
                    }
                            ...
                            buttons: [
                                {
                                    ...
                                    handler: function () {
                                        ...
                                        selection = grid.getSelectionModel().select(0);    
                                    }
                                },
                                {
