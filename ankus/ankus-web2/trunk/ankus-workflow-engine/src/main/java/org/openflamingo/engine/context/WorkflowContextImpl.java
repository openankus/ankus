...
public class WorkflowContextImpl implements WorkflowContext {
    ...
    public WorkflowContextImpl(SchedulerContext schedulerContext) {
        try {
            ...
            this.workflowDomain = (Workflow) getJobDataMap(schedulerContext).get(JobVariableWorkFlow.WORKFLOW);
            ...
            this.globalVariables.put("JOB_TYPE", this.getJobDataMap(schedulerContext).getString(JobVariableWorkFlow.JOB_TYPE));
            this.globalVariables.put("RANDOM", JVMIDUtils.generateUUID());
            this.globalVariables.put("YYYYMMDD", DateUtils.getCurrentYyyymmdd());
            this.globalVariables.put("NAMENODE", ((HadoopCluster) this.getJobDataMap(schedulerContext).get(JobVariableWorkFlow.HADOOP_CLUSTER)).getHdfsUrl());

            String jobStringId = this.getJobDataMap(schedulerContext).getString(JobVariableWorkFlow.JOB_ID);
            ...
            if (StringUtils.isEmpty(jobStringId)) { // FIXME : Pig Job
                ...
                jobDataMap.put(JobVariableWorkFlow.JOB_ID, jobId);
                this.getJobDataMap(schedulerContext).getString(JobVariableWorkFlow.JOB_ID);
                this.globalVariables.put("JOB_STRING_ID", jobId);
            } else {
                this.globalVariables.put("JOB_STRING_ID", this.getJobDataMap(schedulerContext).getString(JobVariableWorkFlow.JOB_ID));
            }

            if (this.getJobDataMap(schedulerContext).get(JobVariableWorkFlow.CURRENT) == null) {
                jobDataMap.put(JobVariableWorkFlow.CURRENT, new Date());
            }

            // FIXME Hadoop/S3 관련 정보도 주입 필요.

            // Scheduler Context에 관련 정보를 보관한다.
            this.setObject(JobVariableWorkFlow.ACTION_CONTEXT, actionContextMap);
            this.setObject(JobVariableWorkFlow.WORKFLOW, workflowDomain);
            this.setObject(JobVariableWorkFlow.PLANNER, executionPlanner);
            this.setObject(JobVariableWorkFlow.WORKFLOW_VARIABLES, getEvaluatedProperties(getGlobalVariables()));

            logger.debug("워크플로우 '{}({})'을 실행합니다.", workflowDomain.getWorkflowName(), workflowDomain.getId());
        } catch (Exception ex) {
            this.setObject(JobVariableWorkFlow.WORKFLOW_EXCEPTION, ex);
            changeState(State.FAIL);

            String jobId = (String) schedulerContext.getJobExecutionContext().getMergedJobDataMap().get(JobVariableWorkFlow.JOB_ID);
            Workflow workflow = (Workflow) schedulerContext.getJobExecutionContext().getMergedJobDataMap().get(JobVariableWorkFlow.WORKFLOW);
    ...
    public String getCurrentActionName() {
        return (String) this.getSchedulerContext().get(JobVariableWorkFlow.ACTION_CURRENT);
    }

    @Override
    public String getCurrentActionDescription() {
        return (String) this.getSchedulerContext().get(JobVariableWorkFlow.ACTION_CURRENT_DESC);
    }

    @Override
    public Map<String, ActionContext> getActionContexts() {
        return (Map<String, ActionContext>) this.getSchedulerContext().get(JobVariableWorkFlow.ACTION_CONTEXT);
    }
    ...
    public Exception getException() {
        return (Exception) this.getObject(JobVariableWorkFlow.WORKFLOW_EXCEPTION);
    }
    ...
    protected WorkflowHistory getWorkflowHistory() {
        return (WorkflowHistory) this.getSchedulerContext().get(JobVariableWorkFlow.WORKFLOW_HISTORY);
    }
