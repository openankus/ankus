 ...
 */
@Component
public class RunningWorkflowStateHandler implements WorkflowStateHandler {

    /**
     * SLF4J Logging
     */
    private Logger logger = LoggerFactory.getLogger(RunningWorkflowStateHandler.class);

    @Override
    public WorkflowHistory changeStatus(WorkflowContext workflowContext) {
        logger.trace("Workflow의 상태를 Running 상태로 전환합니다.");
        ...
        logger.trace("Workflow '{}({})'의 상태를 Running으로 변경하였습니다.", workflowHistory.getWorkflowName(), workflowHistory.getId());
