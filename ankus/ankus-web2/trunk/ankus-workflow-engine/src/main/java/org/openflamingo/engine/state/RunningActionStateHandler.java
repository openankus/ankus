 ...
 */
@Component
public class RunningActionStateHandler implements ActionStateHandler {

    /**
     * SLF4J Logging
     */
    private Logger logger = LoggerFactory.getLogger(RunningActionStateHandler.class);

    @Override
    public ActionHistory changeStatus(ActionContext actionContext) {
        ActionHistory actionHistory = (ActionHistory) actionContext.getObject(JobVariableWorkFlow.ACTION_HISTORY);
        actionHistory.setStatus(State.RUNNING);
        ...
        logger.trace("Action '{}({})'의 상태를 Running으로 변경하였습니다.", actionHistory.getActionName(), actionHistory.getId());
