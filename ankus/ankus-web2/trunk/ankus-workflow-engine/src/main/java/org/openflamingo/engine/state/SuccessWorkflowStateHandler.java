 ...
 */
@Component
public class SuccessWorkflowStateHandler implements WorkflowStateHandler {

    /**
     * SLF4J Logging
     */
    private Logger logger = LoggerFactory.getLogger(SuccessWorkflowStateHandler.class);

    @Override
    public WorkflowHistory changeStatus(WorkflowContext workflowContext) {
        logger.trace("Workflow의 상태를 Success 상태로 전환합니다.");

        WorkflowHistory workflowHistory = (WorkflowHistory) workflowContext.getObject(JobVariableWorkFlow.WORKFLOW_HISTORY);
        workflowHistory.setStatus(State.SUCCESS);
        ...
        String xml =  workflowContext.getWorkflowDomain().getWorkflowXml();
      
        String job_name = workflowContext.getSchedulerContext().getJobExecutionContext().getJobDetail().getKey().getName();
        workflowContext.getBean(WorkflowHistoryRepository.class).update(workflowHistory);
        workflowContext.setObject(JobVariableWorkFlow.WORKFLOW_HISTORY, workflowHistory);

        logger.trace("Workflow '{}({})'의 상태를 Success으로 변경하였습니다.", workflowHistory.getWorkflowName(), workflowHistory.getId());
