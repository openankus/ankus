...
import org.openflamingo.engine.state.visualization_state_share;
...
public class HistoryServiceImpl extends LocaleSupport implements HistoryService {
    ...
    @Autowired
    private VisualizationHistoryRepository visualizationHistoryRepository;
    ...
    public List<WorkflowHistory> getWorkflowHistories(String startDate, String endDate, String workflowName, String jobType, String username, String status, String orderBy, String desc, int start, int limit) {
        try {
        	...
        	if("VISUALIZATION".equals(jobType)){
        		 workflowHistories = workflowHistoryRepository.selectByConditionViz(startDate, endDate, workflowName, jobType, username, status, orderBy, desc, start, limit);
        	}else{
        		 workflowHistories = workflowHistoryRepository.selectByCondition(startDate, endDate, workflowName, jobType, username, status, orderBy, desc, start, limit);
        	}
    ...
    @Override
    public List<VisualizationHistory> getVisualizationHistoryByJobStringId(String jobStringId) {
    	try {
    		return visualizationHistoryRepository.selectByJobStringId(jobStringId);
    	} catch (Exception ex) {
    		throw new ServiceException(message("S_HISTORY_SERVICE", "CANNOT_LIST_WORKFLOW_HISTORIES"), ex);
    	}
    }
    @Override
    public List<WorkflowHistory> getWorkflowHistoryByJobStringId(String jobStringId) {
    	try {
    		return workflowHistoryRepository.selectByJobStringId(jobStringId);
    	} catch (Exception ex) {
    		throw new ServiceException(message("S_HISTORY_SERVICE", "CANNOT_LIST_WORKFLOW_HISTORIES"), ex);
    	}
    }
