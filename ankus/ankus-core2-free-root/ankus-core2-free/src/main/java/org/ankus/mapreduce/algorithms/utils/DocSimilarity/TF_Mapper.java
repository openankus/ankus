...
public class TF_Mapper  extends Mapper<Object, Text, Text, Text>{
  ...
  //  int m_indexArr[];
   ...
   // int m_classIndex;
	...
	//term bases
	/*
	protected void map_term(Object key, Text value, Context context)// throws IOException, InterruptedException
	{
		String[] tokens = value.toString().replaceAll("[\\W&&[^\\s]]", "").split("\\W+");
		...
		try
		{
			for(int i = 0; i < tokens.length; i++)
			{
				for(int t = 0; t < term.size(); t++)
				{
					...
					{
						if(term.get(t).equalsIgnoreCase(tokens[i])== true)
						{
							context.write(new Text(tokens[i]) , new Text("1"));							
						}						
					}
				}
